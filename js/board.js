...
const loadTokenImages = async () => {
    console.log("Starting to load token images...");
    
    // Get all role keys (should be uppercase)
    const roles = Object.keys(PLAYER_ROLES);
    console.log(`Available roles for token loading: ${roles.join(', ')}`);
    
    // Create a role-to-file mapping for diagnostics
    const roleToImageMapping = {};
    roles.forEach(role => {
        const tokenFile = PLAYER_ROLES[role]?.token;
        roleToImageMapping[role] = `${TOKEN_DIR}/${tokenFile}`;
    });
    console.log("Role to image mapping:", roleToImageMapping);
    
    const promises = roles.map(role => new Promise((resolve) => {
        // Skip if role is not found in PLAYER_ROLES
        if (!PLAYER_ROLES[role]) {
            console.warn(`Role '${role}' not found in PLAYER_ROLES`);
            resolve();
            return;
        }
        
        // Skip if token path is not defined
        const tokenPath = PLAYER_ROLES[role]?.token;
        if (!tokenPath) {
            console.warn(`No token path defined for role: ${role}`);
            resolve();
            return;
        }
        
        const fullPath = `${TOKEN_DIR}/${tokenPath}`;
        console.log(`Loading token image for role ${role}: ${fullPath}`);
        
        const img = new Image();
        img.onload = () => {
            console.log(`✅ Successfully loaded token image for ${role}: ${fullPath}`);
            boardState.playerTokenImages[role] = img;
            resolve();
        };
        img.onerror = (e) => {
            console.warn(`❌ Failed to load token image for ${role}: ${fullPath}`, e);
            
            // Try alternate paths as fallback
            const alternativePath = `${TOKEN_DIR}/${role.charAt(0)}.png`;
            console.log(`Trying alternative path: ${alternativePath}`);
            
            const altImg = new Image();
            altImg.onload = () => {
                console.log(`✅ Successfully loaded alternative token image for ${role}: ${alternativePath}`);
                boardState.playerTokenImages[role] = altImg;
                resolve();
            };
            altImg.onerror = () => {
                console.error(`❌ Also failed to load alternative image for ${role}: ${alternativePath}`);
                // Resolve anyway, use fallback
                resolve();
            };
    ...
    try {
        ...
        console.log("All token image loading attempts completed");
        
        // Log what images were actually loaded
        const loadedRoles = Object.keys(boardState.playerTokenImages);
        console.log(`Loaded token images for ${loadedRoles.length}/${roles.length} roles:`, loadedRoles);
    } catch (error) {
...
const drawPlayerToken = (player) => {
    if (!boardState.ctx || !player || !boardState.scale || boardState.scale <= 0) {
        console.warn("Cannot draw player token: missing context, player, or scale");
        return;
    }
    
    // Check for valid coordinates in either property
    let coords = null;
    if (player.currentCoords && player.currentCoords.x !== undefined && player.currentCoords.y !== undefined) {
        coords = player.currentCoords;
    } else if (player.coords && player.coords.x !== undefined && player.coords.y !== undefined) {
        coords = player.coords;
        // Also update currentCoords for future consistency
        player.currentCoords = { ...player.coords };
    } else {
        console.warn(`Cannot draw player token: player ${player.id} has no valid coordinates`, player);
        return;
    }
    ...
    const canvasY = coords.y * scale;
    const role = player.role; 
    const tokenSizeOnCanvas = TOKEN_SIZE * scale; 
    const radius = tokenSizeOnCanvas / 2;
    
    // Get token image (handle both case formats)
    let tokenImage = null;
    if (role && boardState.playerTokenImages[role]) {
        tokenImage = boardState.playerTokenImages[role];
    } else if (role && boardState.playerTokenImages[role.toUpperCase()]) {
        // Try uppercase version as fallback
        tokenImage = boardState.playerTokenImages[role.toUpperCase()];
    }
    ...
    if (tokenImage && tokenImage.complete && tokenImage.naturalWidth > 0) {
        ...
        console.warn(`No valid token image for player ${player.id} with role ${role}, using fallback`);
        drawFallbackToken(ctx, canvasX, canvasY, radius, player);
    }
};
...
const drawAllPlayerTokens = () => {
    ...
    if (!players || players.length === 0) {
        console.warn("No players found to draw tokens");
        return;
    }
    
    console.log(`Drawing tokens for ${players.length} players`);
    players.forEach(player => {
        if (player) {
            // We'll let drawPlayerToken handle the coordinate checks
...
export const drawBoard = (skipPlayers = false) => {
    if (!boardState.ctx || !boardState.boardImage || !boardState.isInitialized || !boardState.scale || boardState.scale <= 0) {
        return; 
    }
    ...
    // Only draw player tokens if not skipped
    if (!skipPlayers) {
        drawAllPlayerTokens();
    }
...
export const setupBoard = async () => {
    ...
    try {
        ...
        // Add event listener for requestBoardRedraw events
        document.addEventListener('requestBoardRedraw', (event) => {
            const skipPlayers = event.detail?.skipPlayers || false;
            drawBoard(skipPlayers);
        });
